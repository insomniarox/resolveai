...
The Next.js app rewrites `/api` to FastAPI. Northflank exposes Next.js publicly and keeps API and PostgreSQL on the private project network. The private API is reachable through the public Next.js `/api` rewrite. FastAPI therefore enforces a Bearer token for runbook uploads; the private network alone would not protect that route. See [README.md](../README.md#northflank-deployment-and-seed-updates) for seed-only updates when a PostgreSQL user cannot run the table DDL in `database/init.sql`.
